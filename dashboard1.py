...
app.layout = html.Div(
    ...
    dbc.Row(dbc.Col(dcc.Dropdown(id = 'food_dropdown', options = items_list, placeholder = 'Select a food', value = 'Lemons and limes'), width = 6), justify='center', align='center'),
    dbc.Row(
    ...
    dbc.Row(
        ...
        dbc.Col(dcc.Slider(df['Year'].min(), df['Year'].max(), step = None, value = df['Year'].min(), marks={str(year): str(year) for year in df['Year'].unique()}, id='year_slider_comparison'), width = {'size': 3, 'offset': 0}, style = {'margin-right': 5})], justify='center')
    ]
