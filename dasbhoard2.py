...
app.layout = html.Div(
    ...
    dbc.Row(
        [dbc.Col(dcc.Dropdown(id = 'food_dropdown', options = items_list, placeholder = 'Select a food', value = 'Lemons and limes'), width = 4),
    ...
    dbc.Row(dbc.Col(dcc.Slider(df['Year'].min(), df['Year'].max(),id='year_slider_sankey', step = None, value = df['Year'].min(), marks={str(year): str(year) for year in df['Year'].unique()}), width = 6, style={'margin-bottom': 30}), justify = 'center', align = 'center'),
    dbc.Row(
    ...
    dbc.Row(  
        [dbc.Col(dcc.RadioItems(id = 'valor_imp_exp', options = ['Imports', 'Exports'], value = 'Imports', inputStyle={"margin-left": "20px", "margin-right": "5px"}), width = 3),
        dbc.Col(dcc.Slider(df['Year'].min(), df['Year'].max(),id='year_slider_imp_exp', step = None, value = df['Year'].min(), marks={str(year): str(year) for year in df['Year'].unique()}), width = 5)],
        justify = 'center', align = 'center'),
    dbc.Row(
    ...
    dbc.Row(  
        [dbc.Col(dcc.RadioItems(id = 'valor_imp_emissions', options = ['Production Emissions', 'Transport Emissions', 'Total Emissions'], value = 'Production Emissions', inputStyle={"margin-left": "20px", "margin-right": "5px"}), width = 5),
        dbc.Col(dcc.Slider(df['Year'].min(), df['Year'].max(),id='year_slider_imp_emissions', step = None, value = df['Year'].min(), marks={str(year): str(year) for year in df['Year'].unique()}), width = 5)],
        justify = 'center', align = 'center'),
    ])
